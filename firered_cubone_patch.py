...
def make_ips(original: bytes, patched: bytes) -> bytes:
    ...
    while position < len(original):
        ...
        while position < len(original) and original[position] != patched[position]:
            position += 1
        changed = patched[start:position]
        for chunk_start in range(0, len(changed), 0xFFFF):
            chunk = changed[chunk_start : chunk_start + 0xFFFF]
            output.extend((start + chunk_start).to_bytes(3, "big"))
